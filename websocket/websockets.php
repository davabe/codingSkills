...
abstract class WebSocketServer {
  ...
  public function run() {
        ...
        //$this->stdout("questi sono this->socket this->master al primo run(). " .var_dump($this->sockets).$this->master);
    while(true) {
        //$this->stdout("questi sono this->socket this->master al primo run(). " .var_dump($this->sockets).$this->master);
      ...
      foreach ($read as $socket) {
        if ($socket == $this->master) {
          ...
          if ($client < 0) {
            ...
            //$this->stdout("La sessione è  " . $_SESSION['dati']['idUser']);
          }
        } 
        else {
          $numBytes = @socket_recv($socket,$buffer,$this->maxBufferSize,0); 
            
          $this->stdout("questo è il buffer");
          //var_dump($buffer);
          //echo "\n";
          //echo "questo è il numbyte $numBytes";
          //echo "\n";
          
          if ($numBytes === false) {
              $this->stdout("questo è l'ultimo errore".socket_last_error($socket));
              ...
              if (($message = $this->deframe($buffer, $user)) !== FALSE) {
                if($user->hasSentClose) {
                  ...
                  $this->stdout("Client disconnected. Sent close: " . $socket);
                    ...
                    $this->stdout("questo è il messaggio " . $message);
  ...
  protected function disconnect($socket, $triggerClosed = true) {
    ...
    if ($foundUser !== null) {
      ...
      if(isset($disconnectedUser->idUser)){
      unset($this->array_user_id[$disconnectedUser->idUser]);// per cancellare sul array custom l'user
      //echo "array user custom cancellato ";
      
      }
      
      $this->users = array_values($this->users);
      $message = $this->frame('', $disconnectedUser, 'close');
      //echo "tranquillo lo cancello e questo è il messagio";
      //$this->stdout($message);
  ...
  protected function doHandshake($user, $buffer) {
    ...
    $this->stdout("questa è la risposta del server handeshale".$handshakeResponse);
    socket_write($user->socket,$handshakeResponse,strlen($handshakeResponse));
    $this->connected($user);
  }
  ...
  protected function deframe($message, &$user) {
    ...
    //echo "questo è header del messaggio nella funzione defrage\n";
    //var_dump($headers);
    //echo "\n";
    $pongReply = false;
    $willClose = false;
